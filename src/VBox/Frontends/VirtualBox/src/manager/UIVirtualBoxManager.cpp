/* $Id: UIVirtualBoxManager.cpp 111881 2025-11-26 09:59:07Z sergey.dubov@oracle.com $ */
/** @file
...
void UIVirtualBoxManager::prepareMenuBar()
{
    ...
    /* OpenHuizeBox "Create Audit VM" was a standalone dialog that pre-dated the
        ...
        if (vms.isEmpty()) {
            QMessageBox::information(0, QString::fromUtf8("OpenHuizeBox"),
                QString::fromUtf8("No VMs are registered yet. Create one via Machine &rarr; New, "
                                  "then open Settings &rarr; OpenHuizeBox to configure the hardware identity."));
            return;
        }
        bool ok = false;
